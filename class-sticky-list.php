...
class StickyList extends GFAddOn {
        ...
        if ( isset( $settings['enable_list'] ) && true == $settings['enable_list'] ) {
            ...
            if ( $position == - 1 ) { ?>
                ...
                <li class="list_setting">
                    <label for="field_list_value" class="section_label">
                        <?php esc_html_e( 'Sticky List', 'sticky-list' ); ?>
                    </label>
                    <input type="checkbox" id="field_list_value" onclick="SetFieldProperty('stickylistField', this.checked);"/>
                    <label class="inline" for="field_list_value">
                        <?php esc_html_e( 'Show in list', 'sticky-list' ); ?>
                        <?php gform_tooltip( 'form_field_list_value' ) ?>
                    </label>
                    <br>
                    <input type="checkbox" id="field_nowrap_value" onclick="SetFieldProperty('stickylistFieldNoWrap', this.checked);"/>
                    <label class="inline" for="field_nowrap_value">
                        <?php esc_html_e( "Don't wrap text from this field", 'sticky-list' ); ?>
                        <?php gform_tooltip( 'form_field_nowrap_value' ) ?>
                    </label>
                    <br>
                    <label class="inline" for="field_list_text_value">
                        <?php esc_html_e( 'Column label', 'sticky-list' ); ?>
                        <?php gform_tooltip( 'form_field_text_value' ) ?>
                    </label>
                    <br>
                    <input class="fieldwidth-3" type="text" id="field_list_text_value" onkeyup="SetFieldProperty('stickylistFieldLabel', this.value);"/>
                </li>
	...
	function add_stickylist_tooltips( $tooltips ) {
		...
		$tooltips['form_field_nowrap_value'] = __( "<h6>Don't wrap whitespace</h6>Check this box to prevent wrapping of text from this field", 'sticky-list' );
				...
				foreach ( $entries as $entry ) {
					...
					foreach ( $form['fields'] as $field ) {
						...
						if ( $field->stickylistField ) {
							...
							$nowrap = '';
							if ( $field->stickylistFieldNoWrap ) {
							...
							elseif ( in_array( $field->get_input_type(), array( 'fileupload', 'post_image' ) ) ) {
		...
		if ( isset( $_POST['mode'] ) == 'edit' || isset( $_POST['mode'] ) == 'view' || isset( $_POST['mode'] ) == 'duplicate' ) {
			...
			if ( ! is_wp_error( $form_fields ) && $form_fields['status'] == 'active' ) {
				...
				if ( $form_fields['created_by'] == $this->stickylist_get_current_user() || current_user_can( 'edit_others_posts' ) || current_user_can( 'stickylist_edit_entries' ) || $_POST['mode'] == 'view' || $_POST['mode'] == 'duplicate' ) {
					...
					if ( rgpost( 'gform_submit' ) == $form_id || isset( $_POST['mode'] ) ) {
					...
					// Get update text
					if ( isset( $settings['update_text'] ) ) {
						$update_text = $settings['update_text'];
					} else {
						$update_text = '';
					} ?>
	...
	public function form_settings_fields( $form ) {
		...
		if ( count( $posts_array ) > 1000 ) {
			$embedd_post_page_label = __( 'Embed page/post', 'sticky-list' ) . '<br><small>' . __( '<strong>NOTE</strong>: Only the latest 1000 items are shown. Use "custom url" below if your desired post is not listed.', 'sticky-list' ) . '</small>';
		} else {
			$embedd_post_page_label = __( 'Embed page/post', 'sticky-list' );
		}
		...
		foreach ( $form['fields'] as $key => $value ) {
			...
			$this->log_debug( __METHOD__ . '(): value = ' . print_r( $value, 1 ) );
	...
	public function styles() {
		$styles = array(
			array(
				'handle'  => 'sticky-list_admin_styles',
